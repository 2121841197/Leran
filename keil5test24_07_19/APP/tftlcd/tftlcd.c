...
void LCD_ShowFontHZ_KT(u16 x, u16 y, u8 *cn)
{
	u8 i, j, wordNum;
	u16 color;
	u16 x0=x; 
	...
	while (*cn != '\0')
	{
		for (wordNum=0; wordNum<200; wordNum++)
		{	//wordNum扫描字库的字数
			if ((CnChar40x37[wordNum].Index[0]==*cn)
			     &&(CnChar40x37[wordNum].Index[1]==*(cn+1)))
			{
				for(i=0; i<185; i++) 
				{	//MSK的位数
					color=CnChar40x37[wordNum].Msk[i];
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
...
void LCD_ShowFontHZ_ST(u16 x, u16 y, u8 *cn)
{
	u8 i, j, wordNum;
	u16 color;
	u16 x0=x; 
	...
	while (*cn != '\0')
	{
		for (wordNum=0; wordNum<200; wordNum++)
		{	//wordNum扫描字库的字数
			if ((CnChar32x27[wordNum].Index[0]==*cn)
			     &&(CnChar32x27[wordNum].Index[1]==*(cn+1)))
			{
				for(i=0; i<108; i++) 
				{	//MSK的位数
					color=CnChar32x27[wordNum].Msk[i];
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
...
void LCD_ShowFontHZ_LS(u16 x, u16 y, u8 *cn)
{
	u16 i, j, wordNum;
	u16 color;
	u16 x0=x; 
	...
	while (*cn != '\0')
	{
		for (wordNum=0; wordNum<200; wordNum++)
		{	//wordNum扫描字库的字数
			if ((CnChar64x64[wordNum].Index[0]==*cn)
			     &&(CnChar64x64[wordNum].Index[1]==*(cn+1)))
			{
				for(i=0; i<512; i++) 
				{	//MSK的位数
					color=CnChar64x64[wordNum].Msk[i];
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
